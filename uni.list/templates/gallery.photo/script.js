...
$(function(){
	$(document).ready(function () {
		...
		if(urlIdPos != -1) {
			var imageID = url.slice(urlIdPos + 4);
			$('.project_list__item .image[data-link-id="'+imageID+'"]').trigger('click');
		}
	});
	...
	galleryTop = new Swiper('.gallery-top', {
				...
				onInit: function(swiper){
					//$('.photo-name').html(gallery.find('.gallery-top .swiper-slide').eq(swiper.activeIndex).data('name'));
					////$('#pNum').html(swiper.activeIndex+1);
	...
	galleryThumbs = new Swiper('.gallery-thumbs', {
		...
		$('.popup-gallery .project_list__item a').click(function(){ 
			slide = parseInt($(this).data('num'));
				$('#galleryModal').modal();   
				return false;
		});
